...
- 管理者による校内マップ画像の登録
- 画像の縦横比を維持するレスポンシブマップと、同一場所の人数マーカー
- 場所ごとのQRコード画像ダウンロード（読み取り時に場所を事前選択）
- Firebase AI Logic経由の居場所案内・一般質問対応Geminiチャット（`gemini-3.5-flash-lite`）
- 質問形式を限定しない、実名の共有データを使ったプロトタイプ回答
...
プロトタイプでは質問形式を限定せず、利用者の入力文と、共有中の教職員名・選択場所・在席状況・対応予定時刻・情報の鮮度をそのままGeminiへ送信します。Firestore全体、メールアドレス、ユーザーID、ひとこと欄は送信しません。居場所以外の一般質問にも対応します。実運用前に匿名化、利用規約、保存・監査方針、利用する別AIまたはローカルモデル、有料構成を再検討してください。
